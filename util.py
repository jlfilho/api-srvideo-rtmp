...
def start_ffmpeg_writer(out_filename, width, height):
    ...
    args = (
        ffmpeg
        .input('pipe:', format='rawvideo', vcodec='rawvideo', pix_fmt='rgb24', r='{}'.format(30),s='{}x{}'.format(width, height))
        .input.audio('pipe:', )
        .output(out_filename, pix_fmt='yuv420p',tune='zerolatency',vcodec='libx264',preset='ultrafast',format='flv')
        .global_args('-re')
        .overwrite_output()
        .compile()
    )
